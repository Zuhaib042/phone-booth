...
The API exposes `GET /health/live` plus versioned identity, matchmaking,
real-time, private-chat, reporting, mute, and block routes under `/v1`.
Identity is disabled unless an environment selects the guarded development
provider or configures Sign in with Apple. Backend environment variables and
